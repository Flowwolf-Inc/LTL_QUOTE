...
class ArcBestCarrierAdapter(BaseCarrierAdapter):
	...
	def get_rates(self, request: ShipmentRequest) -> CarrierRateQuote:
		...
		try:
			...
			return self._parse_rate_xml(response.content, response.text, params)
	...
	def _extract_request_fields(self, request) -> dict:
		"""Read weight/class/qty from parent doc fields; optional items[] is fallback only."""
		if isinstance(request, ShipmentRequest):
			return {
				...
				"unit_type": "PLT",
				"accessorials": request.accessorial_codes or [],
			}
		...
		return {
			...
			"destination_state": request.get("destination_state"),
			...
			"freight_class": request.get("freight_class") or first_item.get("freight_class") or "50.0",
	...
	def _build_rate_params(self, request) -> dict:
		...
		accessorials = {str(code).upper() for code in (fields.get("accessorials") or [])}
		...
		params = {
			"DL": "2",
			"ID": self.api_id,
			"ShipCity": fields.get("origin_city") or origin_zip,
			"ShipState": fields.get("origin_state") or "",
			"ShipZip": origin_zip,
			"ShipCountry": fields.get("origin_country") or "US",
			"ConsCity": fields.get("destination_city") or destination_zip,
		...
		if "LIFTGATE" in accessorials:
			params["Acc_GRD_DEL"] = "Y"
		if "RESIDENTIAL" in accessorials:
			params["Acc_RDEL"] = "Y"
		if "HAZMAT" in accessorials:
			params["Acc_HAZ"] = "Y"
